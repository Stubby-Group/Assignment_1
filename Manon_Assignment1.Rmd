...
```{r}
customPrior <- function(priorMean, priorSD, teacherInitial, plotTitle){
  dens <- 100
  YAS_grid <- tibble(p_grid = seq(from = 0, to = 1, length.out = dens), 
                 prior = dnorm(x = 0, mean = priorMean, sd = priorSD)) %>%
    ...
    d <- data.frame(grid = YAS_grid$p_grid, posterior = YAS_grid$posterior, prior = YAS_grid$prior, likelihood = YAS_grid$likelihood)
